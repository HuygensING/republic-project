...
# reading in entity references

def read_entity_references(unaggregated_data_tsv):

    references = { }

    with open(unaggregated_data_tsv) as file:
        for row in csv.DictReader(file, delimiter='\t', quoting=csv.QUOTE_NONE):
            er = EntityReference(**row)
            if er.tag_text in references:
                references[er.tag_text].append(er)
            else:
                references[er.tag_text] = [er]

    print(f'{sum([len(refs) for refs in references.values()])} references read; {len(references)} unique')


    corrections = { }
    variants = { kw['label']: { } for kw in spelling_keywords }

    def find_corrections(old):
        new, vs = harmonise_spelling(old)
        if old != new:
            return old, new, vs
    def apply_corrections(lst):
        for (old, new, vs) in lst:
            corrections[old] = new
            for (p,r) in vs:
                if p in variants[r]:
                    variants[r][p] += 1
    ...
    search_in_parallel(references.keys(), find_corrections, apply_corrections)

    print(f'{len(corrections)} references corrected')

    results = [ ]

    for s in references:
        for er in references[s]:
            results.append((corrections.get(s,s), jaargangen[er.inv], er))
