...
class FuzzyPersonNameSearcher(FuzzyContextSearcher):
    ...
    def find_person_names_in_context(self, context, person_name_patterns=None, context_patterns=None):
        name_matches = []
        ...
        for name_re_match, search_context in self.find_patterns_in_context(context, person_name_patterns, context_patterns):
            parse_person_name_in_context_matches(name_re_match, search_context, context, name_matches)
        return name_matches

    def find_person_names_in_text(self, text, person_name_patterns=None):
        ...
        for name1 in close_distance_names:
            ...
            for name2 in close_distance_names[name1]:
                ...
                if self.filter_distant_names(first_name1, first_name2, max_distance_ratio):
                    self.remove_close_distance_names(close_distance_names, name1, name2)
    ...
    def filter_distant_names(self, name1, name2, max_distance_ratio):
        distance = self.score_levenshtein_distance(name1, name2)
    ...
    def remove_close_distance_names(self, close_distance_names, name1, name2):
...
def parse_person_name_in_context_matches(name_re_match, search_context, context, name_matches):
    ...
    for pattern_group_index in search_context["group_indices"]:
        ...
        whole_match = whole_match[whole_match.index(name_re_match.group(pattern_group_index)) + len(name_match["name_string"]):]
...
if __name__ == "__main__":
    from name_patterns import dutch_person_name_patterns as name_patterns
