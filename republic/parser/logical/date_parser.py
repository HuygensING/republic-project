...
def check_line_starts_with_week_day_name(line: pdm.PageXMLTextLine):
    for set_version in week_day_names:
        for week_day_name in week_day_names[set_version]:
            if line.text.startswith(week_day_name):
                return True
    return False
...
def get_session_date_lines_from_pages(pages):
    ...
    return filter_session_date_lines(date_lines)


def filter_session_date_lines(date_lines):
    session_date_lines = []
    for line in date_lines:
        if check_line_starts_with_week_day_name(line):
...
def get_session_date_line_token_length(session_date_lines):
    ...
    print('line_token_lengths:', Counter(line_token_lengths))
    return max(line_token_lengths, key=line_token_lengths.count)


def get_standard_date_line_pos_tokens(standard_lines):
...
def get_session_date_line_structure(session_date_lines: List[pdm.PageXMLTextLine],
    ...
    pos_tokens = get_standard_date_line_pos_tokens(standard_lines)
